...
Data = pd.concat([Data['Average_Arousal_UK'], Data['Average_Arousal_US'], Data['Average_Arousal_JP'],
...
#counts_ones=[]
for d in range(3):
    Data_Asian[:,d] = np.where(Data_Asian[:,d] > avg_arousals[d], 1, 0)          #Arousal = 1 if >avg_arousals[d], 0 otherwise
    Data_Western[:,d] = np.where(Data_Western[:,d] > avg_arousals[d], 1, 0)
            ...
            for c in range(2):
                ...
                if i % 10 == 9:
                    ...
                    if j == 3 and c == 1:
                        ...
                        # plt.legend()
                        fig.tight_layout()
                        fig.savefig(f"Arousal ROC curves_it {i}_ba.png")
                    ...
                    if j == 3 and c == 1:
                        ...
                        # plt.legend()
                        fig2.tight_layout()
                        fig2.savefig("Arousal Confusion matrices_it {}_ba.png".format(i))
